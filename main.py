...
def search_young(id):
    ...
    for each in data:
        print("\t",each)
    new = int(input("Enter ID you want to send request or 0 to skip"))
    if new:
        with conn:
            c.execute("INSERT INTO REQUESTS_TO_Y VALUES(?,?)", (id, new))
        print("\tRequest send successfully\n")
        c.execute("SELECT * FROM REQUESTS_TO_Y WHERE Y_ID=(?)",(new,))
        data = c.fetchall()
        print(data)
    ...
    with conn:
        ...
        if len(data):
            ...
            if new != 0:
                ...
                c.execute("DELETE FROM REQUESTS_TO_E WHERE E_ID =(?) AND Y_ID = (?)",(id,new))
        else:
...
def send_req(Y_id):
    ...
    if data:
        ...
        if req_id:
            c.execute("INSERT INTO REQUESTS_TO_E VALUES(?,?)",(req_id, Y_id))
            print("\tRequest send successfully\n")
    else:
...
def see_y_req(Y_id, no_e_looking):
    with conn:
        c.execute("SELECT Y_ID FROM REQUESTS_TO_Y WHERE Y_ID = (?)", (Y_id,))
        data = c.fetchall()
        if len(data):
            for i in range(len(data)):
                with conn:
            ...
            if new != 0:
                if no_e_looking < 4:
                    with conn:
                        c.execute("UPDATE ELDER SET CARE_TAKER = (?) WHERE ID = (?)",(Y_id, new))
                        c.execute("UPDATE YOUNG SET LOOKING_AFTER = (LOOKING_AFTER+1) WHERE ID = (?)",(Y_id,))
                        c.execute("INSERT INTO UNDER_Y VALUES(?,?)",(Y_id,new))
                    print("\tNow you have one more elder to look after : ")
                    c.execute("SELECT ID, NAME FROM YOUNG WHERE ID = (?)",(new,))
                    data = c.fetchone()
                    print("\tID : ",data[0],"\tName : ",data[1],"\n")
                    c.execute("DELETE FROM REQUESTS_TO_Y WHERE Y_ID = (?) AND E_ID = (?)", (Y_id,new))
